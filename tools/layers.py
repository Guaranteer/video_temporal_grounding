...
def linear_layer(input_ts, output_dim, scope_name):
    with tf.variable_scope(scope_name):
        ...
        b = tf.get_variable('b', shape=[output_dim], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
...
def scalar_attention_layer(input_ts_list, context_ts, scope_name):
    step = len(input_ts_list)
    input_dim = input_ts_list[0].shape[1]
    context_dim = context_ts.shape[1]
    with tf.variable_scope(scope_name):
        w_c = tf.get_variable('w_c', shape=[context_dim, 1], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        w_i = tf.get_variable('w_i', shape=[input_dim, 1], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        ...
        w_a = tf.get_variable('w_a', shape=[step], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
...
def bilinear_attention_layer(input_ts, context_ts, scope_name):
    ...
    with tf.variable_scope(scope_name):
        ...
        p = tf.get_variable('p', shape=[input_dim, context_dim], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
...
def collective_matrix_attention_layer(input_ts, context_ts, attention_dim, scope_name, context_len=None, use_maxpooling=True):
    ...
    for i in range(max_context_len):
        attention_output, attention_score = matrix_attention_layer(input_ts, unstack_context[i], attention_dim, scope_name+'_'+str(i), weights_only=False)
...
def embedding_layer(input_ts, output_dim, scope_name):
    with tf.variable_scope(scope_name):
        w_e = tf.get_variable('w_e', shape=[input_ts.shape[1], output_dim], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        embedding_output = tf.nn.embedding_lookup(w_e, input_ts)
        return embedding_output

def w2v_embedding_layer(input_ts, filename, scope_name):
    with tf.variable_scope(scope_name):
        embeddings = pkl.load(open(filename))
