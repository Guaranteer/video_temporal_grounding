...
class Model(object):
    ...
    def build_model(self):
        ...
        with tf.variable_scope("Output_Layer"):
            ...
            ground_prod = tf.nn.softmax(layers.mask_logits(self.gt_predict,self.gt_predict))
    ...
    def discriminator(self, v_feature, q_feature, reuse_flag=False):
        ...
        with tf.variable_scope("discriminator" , reuse=reuse_flag):
            v_feature = layers.linear_layer(v_feature, self.hidden_size, scope_name='v_transfer')
            # v_feature = tf.contrib.layers.dropout(v_feature, self.dropout, is_training=self.is_training)

            q_feature = layers.linear_layer(q_feature, self.hidden_size, scope_name='q_transfer')
            ...
            fused_fc = layers.linear_layer(fused_cat, self.hidden_size, scope_name='fc_transfer')
