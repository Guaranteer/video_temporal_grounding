...
class Model(object):
    ...
    def build_model(self):
            ...
            for i in range(1):
                with tf.variable_scope("stack_%s"%i):
                    ...
                    ques_next_layer = conv_utils.conv_encoder_stack(ques_next_layer, [self.hidden_size,self.hidden_size], [3,3],
                                                              {'src': self.dropout,
                                                               'hid': self.dropout}, mode=self.is_training)
                    ques_next_layer = transformer.multihead_attention_v2(queries=ques_next_layer,
                                                                       query_len=self.ques_mask,
                                                                       keys=ques_next_layer,
                                                                       key_len=self.ques_mask,
                                                                       num_units=self.hidden_size,
                                                                       num_heads=4,
                                                                       dropout_rate=1 - self.dropout,
                                                                       is_training=self.is_training,
        ...
        with tf.variable_scope("Context_to_Query_Attention_Layer"):
            ...
            self.q2v = tf.matmul(S_T, frame_embedding)

            fused_q = layers.fusion_layer(ques_embedding,self.q2v,self.hidden_size,'fused_q')
            fused_v = layers.fusion_layer(frame_embedding,self.v2q,self.hidden_size,'fused_v')
            print(fused_q.shape)
            print(fused_v.shape)
            q_feature, _ = layers.weight_attention_layer(fused_q,self.hidden_size,len_mask=self.ques_mask,scope_name='q_feature')
        ...
        with tf.variable_scope("Model_Encoder_Layer"):
            ...
            model_next_layer = conv_utils.linear_mapping(fused_v, self.hidden_size,
                                                         dropout=self.dropout,
