...
class Model(object):
    ...
    def build_model(self):
        ...
        with tf.variable_scope("Frame_Embedding_Encoder_Layer"):
            ...
            for i in range(3):
                with tf.variable_scope("stack_%s"%i):
                    ...
                    frame_next_layer = transformer.feedforward(frame_next_layer, num_units=[2 * self.hidden_size, self.hidden_size])
        ...
        with tf.variable_scope("Model_Encoder_Layer"):
            ...
            for i in range(2):
                with tf.variable_scope("stack_%s" % i):
                    model_next_layer = conv_utils.conv_encoder_stack(model_next_layer, [256,256], [3,3],
                                                              {'src': self.dropout,
                                                               'hid': self.dropout}, mode=self.is_training)
